...
def drawScene(pacman, ghosts):
    ...
    for i in range(len(mazeCopy)):
        for j in range(len(mazeCopy[0])):
            if mazeCopy[i][j] == 'G' or mazeCopy == 'P':
                mazeCopy[i][j] = '0'
...
def evolvePac(population, stage, bestFit, currGen, bestGen, ghosts, numTimeSteps, numSpaces):
    fitnesses = []
    bestDeathStep = 0
    bestCoverage = 0
    member = 0
    for pacman in population:
        ...
        for movement in range(len(pacmanMove)):
            ...
            print("Test: " + str(test))
            print("Member: " + str(member) + "/" + str(len(population)))
            print("Move: " + str(movement) + "/" + str(len(pacmanMove)))
            print("Evolving: ", end='')
            if stage % 2 != 0:
                print("Pacman")
            else:
...
if __name__ == "__main__":
    ...
    for test in range(100):
        ...
        bestGen = -1
        for i in range(numGens):
            fitnesses, bestFit, bestGen = evolveGhosts(ghostPop, 2, bestFit, i, bestGen, currentBestPacman, numTimeSteps, numSpaces)
        ...
        bestGen = -1
        for i in range(numGens):
            fitnesses, bestFit, bestGen = evolvePac(population, 3, bestFit, i, bestGen, currentBestGhosts, numTimeSteps, numSpaces)
            population = newPop(population, fitnesses)
        ...
        bestGen = -1
        for i in range(numGens):
            fitnesses, bestFit, bestGen = evolveGhosts(ghostPop, 4, bestFit, i, bestGen, currentBestPacman, numTimeSteps, numSpaces)
            ghostPop = newGhostPop(ghostPop, fitnesses)
